...
//      ]);
	...
	describe("#dumpToEndpoint", function () {
		it("sends the contents of the db to an endpoint", function () {
			store.save({b: "B"}, noop);

			jasmine.Ajax.useMock();

			store.dumpToEndpoint();
			var request = mostRecentAjaxRequest();

			expect(request.url).toEqual("http://google.com");
			expect(request.data()).toEqual({ store: [ jasmine.any(String) ] });
		});

		it("console logs when successful", function () {
			spyOn(console, "log");
			store.save({b: "B"}, noop);
...
//		expect to see loading spinner
			...
			request.response({
				status: 200,
				responseText: '{"response": {"body": "some body"}}'
			});

//		expect loading spinner to be gone
